...
**lohar** runs inside the VM as PID 1. It's the guest agent — a static binary with zero dependencies (no systemd, no libc, no initramfs). It handles command execution, file operations, PTY sessions, and communicates with the host over a vsock connection using a custom [wire protocol](/v1/docs/under-the-hood/wire-protocol/).
...
For the full picture, see [Architecture Overview](/v1/docs/under-the-hood/architecture/).
