---
title: Users & Auth
description: Multi-tenant model — per-user API keys, resource caps, network
  isolation, sandbox scoping.
slug: v1/docs/managing/users
---
...
Users are created server-side, with `sudo bhatti user create`. The CLI commands all live in [Server & admin](/v1/docs/reference/cli/admin/) — this page covers the model, not the surface.

You won't usually run `user create` for yourself. The install script (`curl -fsSL bhatti.sh/install | sudo bash`) creates an `admin` user during setup and writes its API key to your `~/.bhatti/config.yaml`. The first manual `user create` you run is for adding a teammate — see [Self-hosting → adding teammates](/v1/docs/self-hosting/#adding-teammates).

## How keys work

The server stores only the SHA-256 hash of each API key. The plaintext is shown once at creation and once on rotation; if the user loses it, [`rotate-key`](/v1/docs/reference/cli/admin/user-rotate-key/) is the only path back. There's no recovery.
...
* [`bhatti user create`](/v1/docs/reference/cli/admin/user-create/), [`list`](/v1/docs/reference/cli/admin/user-list/), [`rotate-key`](/v1/docs/reference/cli/admin/user-rotate-key/), [`delete`](/v1/docs/reference/cli/admin/user-delete/)
* [Networking](/v1/docs/under-the-hood/networking/) — bridge layout, iptables rules, IP allocation
