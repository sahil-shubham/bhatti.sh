---
title: Images & Tiers
description: Built-in rootfs tiers, custom-image workflows, and how image
  scoping and sharing work.
slug: v1/docs/managing/images
---
...
The full CLI surface is in [Images reference](/v1/docs/reference/cli/images/). This page covers the tier landscape, the typical workflows, and how scoping/sharing work — none of which is in the per-command pages.
...
| Tier | Adds | Size |
| ---- | ---- | ----:|
| [`minimal`](/v1/docs/managing/tiers/) | (base) | ~200 MB |
| [`browser`](/v1/docs/managing/tiers/browser/) | Chromium, Playwright, Node 22 | ~600 MB |
| [`docker`](/v1/docs/managing/tiers/docker/) | Docker Engine, buildx, compose, binfmt | ~550 MB |
| [`computer`](/v1/docs/managing/tiers/computer/) | XFCE, KasmVNC, Chromium, full desktop | ~1.5 GB |
...
The deep dive for each tier — how its long-running daemons are managed, what env knobs you can pass, sizing, troubleshooting — lives at [Tiers](/v1/docs/managing/tiers/). The rest of this page is about images you build *on top* of those tiers.
...
Only the **rootfs** is captured — persistent volumes are not part of the image. If your stack puts work in a volume, snapshot the volume separately ([`bhatti volume clone`](/v1/docs/reference/cli/volumes/clone/)) and re-attach when stamping.
...
setup, see
[Filesystem](/v1/docs/self-hosting/#filesystem-recommended-btrfs)),
...
See [Self-hosting →
Filesystem](/v1/docs/self-hosting/#filesystem-recommended-btrfs) for
...
This command operates directly on the local SQLite database, so it requires running on the server with DB access (`sudo`). It is not an HTTP API call. There's no "share with everyone" mode — sharing is always with named users. Inspect current shares with `--list`; revoke with [`bhatti image unshare`](/v1/docs/reference/cli/images/unshare/).
...
* [Tiers](/v1/docs/managing/tiers/) — the four built-in starting points, with per-tier deep dives
* [Images reference](/v1/docs/reference/cli/images/) — every command
* [Adding a tier](/v1/docs/contributing/adding-a-tier/) — build a new system tier
