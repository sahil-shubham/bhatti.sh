...
description: How bhatti talks to Firecracker — the actual API calls during boot,
  the jailer setup, rate limiter configuration, and the implementation choices
  behind it.
slug: v1/docs/under-the-hood/engine
---

This page is the lower-level companion to
[Architecture](/v1/docs/under-the-hood/architecture/). It walks through
...
1. **No Firecracker SDK.** We talk directly to FC's Unix-socket
   ...
   [Architecture page](/v1/docs/under-the-hood/architecture/#2-we-talk-to-firecracker-directly-no-sdk);
...
4. **We never use FC's diff snapshots.** `track_dirty_pages: false`
   ...
   is in [Thermal states](/v1/docs/under-the-hood/thermal-states/#why-all-snapshots-are-full).
...
[balloon trick](/v1/docs/under-the-hood/thermal-states/#the-balloon-trick)
during hot→warm doesn't work.

After `InstanceStart`, the host pre-populates the
[ARP cache](/v1/docs/under-the-hood/networking/#the-arp-trick) with the
...
3. Spawns FC as `jailer ... --exec-file /path/to/firecracker --uid <uid> --gid <gid> --id <sandbox_id> --chroot-base-dir <data_dir>/jails ...`.
...
rate limiters
([FC rate limiter docs](https://github.com/firecracker-microvm/firecracker/blob/main/docs/design.md#io-storage-networking-and-rate-limiting)).
...
* [Architecture](/v1/docs/under-the-hood/architecture/) — the bird's-eye
  view that this page sits underneath
* [Thermal states](/v1/docs/under-the-hood/thermal-states/) — what the
  engine does for `Stop`/`Start`/`Pause`/`Resume`
* [Networking](/v1/docs/under-the-hood/networking/) — the bridge, TAP,
