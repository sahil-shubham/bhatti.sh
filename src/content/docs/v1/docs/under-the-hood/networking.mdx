...
description: Per-user bridges in 10.0.x.0/24, the six iptables rules that
  protect the host, the kernel ip= parameter that brings the network up before
  init, and a permanent ARP entry that saves a second of boot time.
slug: v1/docs/under-the-hood/networking
---
...
* [Thermal states](/v1/docs/under-the-hood/thermal-states/) — how the
  network stays consistent across snapshot/resume
* [Lohar](/v1/docs/under-the-hood/lohar-the-blacksmith/) — how the agent
  inside the VM uses TCP to listen for host commands
* [Custom domain](/v1/docs/managing/custom-domain/) — TLS for the public
  proxy, wildcard certs, ACME flow
* [Decisions & learnings](/v1/docs/under-the-hood/decisions/) — the
