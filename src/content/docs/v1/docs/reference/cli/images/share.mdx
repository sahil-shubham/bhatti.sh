...
description: Share an image with specific users. Operates directly on the local
  SQLite database; requires DB access (server-only).
sidebar:
  label: image share
  order: 6
slug: v1/docs/reference/cli/images/share
---
...
`--list` prints the users an image is currently shared with. `--unshare` is its own command, [`bhatti image unshare`](/v1/docs/reference/cli/images/unshare/).
...
* [`bhatti image unshare`](/v1/docs/reference/cli/images/unshare/) — revoke access
* [`bhatti user list`](/v1/docs/reference/cli/admin/user-list/) — find usernames
