...
sidebar:
  label: image pull
  order: 2
slug: v1/docs/reference/cli/images/pull
---
...
`--auth` enables a single-shot HTTP basic auth header (`user:token`). For real private registries with rotating credentials, prefer the [`image import`](/v1/docs/reference/cli/images/import/) flow — `docker pull` handles auth on your machine, then bhatti just receives the tarball.
...
* [`bhatti image import`](/v1/docs/reference/cli/images/import/) — local Docker / tarball import (handles private registries via local auth)
* [`bhatti create --image`](/v1/docs/reference/cli/sandbox/create/) — use the pulled image
* [API: `POST /images/pull`](/v1/docs/reference/api/#pull-an-image)
* [API: `GET /tasks/:id`](/v1/docs/reference/api/#poll-an-async-task)
