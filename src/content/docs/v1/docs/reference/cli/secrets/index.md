...
sidebar:
  label: Overview
  order: 0
slug: v1/docs/reference/cli/secrets
---
...
| Command | Description |
| ------- | ----------- |
| [`bhatti secret set`](/v1/docs/reference/cli/secrets/set/) | Create or update a secret. |
| [`bhatti secret list`](/v1/docs/reference/cli/secrets/list/) | List secret names (values are never returned). |
| [`bhatti secret delete`](/v1/docs/reference/cli/secrets/delete/) | Delete a secret. |
...
When creating from a template, the server uses only the template's pre-configured secret list. Request-side `--secret` flags are silently dropped. See [`bhatti create`](/v1/docs/reference/cli/sandbox/create/) for the workaround.
