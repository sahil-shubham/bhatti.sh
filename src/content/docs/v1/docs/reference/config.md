---
title: Configuration
description: Server and CLI config files, layered loading, environment
  variables, and the data directory layout.
slug: v1/docs/reference/config
---
...
You must set either `tls_cert`+`tls_key` or `acme_email`. A wildcard cert is the right answer for any setup that creates more than a handful of aliases per week. See [Custom domain](/v1/docs/managing/custom-domain/) for the full setup walk-through.

### Backup

For volume backups to S3-compatible storage. Enables [`bhatti volume backup`](/v1/docs/reference/cli/volumes/backup/), `restore`, `backup-list`, and `backup-delete`. Without this block, those endpoints return `501`.
...
`<arch>` is `arm64` on aarch64 hosts, `amd64` on x86\_64.
