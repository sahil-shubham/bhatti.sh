---
title: Docker tier
description: Run Docker Engine inside a microVM. Multi-arch buildx, compose, and the operator UX.
---
...
The custom bhatti kernel enables every flag dockerd needs at runtime — bridge networking, veth, overlayfs, netfilter conntrack and security tables, and `BINFMT_MISC` for cross-arch emulation. See [Kernel](/docs/contributing/kernel/) for the full list and rationale.
...
Both overrides exist because the shim is **not** real systemd. Two specific gaps, both deliberate per [Decisions & learnings](/docs/under-the-hood/decisions/):
...
(Server-side requires either an SSH driver or a TCP-listening dockerd that you publish via `bhatti publish`.) Each platform builds natively on a matching host; buildx writes the multi-arch manifest. bhatti's [warm-wake](/docs/under-the-hood/thermal-states/) (~4 ms) makes this cheap to leave idle between runs.
...
For long-running compose stacks, consider `--keep-hot` on `bhatti create` so the [thermal manager](/docs/under-the-hood/thermal-states/) doesn't pause the VM during idle periods (TCP connections from the public preview URL survive pause, but new connections during pause cost a wake).
...
- [Tiers overview](/docs/managing/tiers/) — what each tier is for
- [Adding a tier](/docs/contributing/adding-a-tier/) — build your own
- [Kernel](/docs/contributing/kernel/) — flags enabled for Docker, why
- [Lohar: systemctl shim](/docs/under-the-hood/lohar-the-blacksmith/) — how unit files are managed without real systemd
- [Decisions & learnings](/docs/under-the-hood/decisions/) — why no socket activation, why no D-Bus
- [Thermal states](/docs/under-the-hood/thermal-states/) — `--keep-hot` and what idle behaviour to expect for long-running stacks
