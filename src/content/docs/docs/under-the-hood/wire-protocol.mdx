...
happens over a binary framing protocol. In production it runs over
vsock — libkrun bridges each guest vsock port to a host Unix socket, and
the daemon dials that socket directly. The protocol itself is engine- and
transport-independent: the entire agent test suite runs on macOS over
`net.Pipe()` (or plain Unix sockets) without any VM or root.
...
5. **One connection per operation.** Most agent calls open a
...
not over the host↔guest vsock connection. They exist so the systemctl
...
Two vsock ports inside each VM, two purposes:

- **Port 1024** (control) — exec, sessions, files, activity queries
- **Port 1025** (forward) — port forwarding / TCP tunneling

`MaxFrameSize` is 1 MB. lohar listens on these vsock ports; libkrun
bridges each to a host Unix socket, and the daemon dials that socket
directly — no CONNECT handshake, no TCP, no TAP device.
...
from the sandbox's persisted state, the token survives — VMs across
daemon restarts use the same token they were created with.
...
and dirty pages in the guest's page cache aren't captured by a snapshot
until they're flushed. Without fsync, the rename is metadata-durable but
the data isn't on the virtio-blk device.
...
- [Decisions & learnings](/docs/under-the-hood/decisions/) — the snapshot
  and networking bugs that shaped these choices
